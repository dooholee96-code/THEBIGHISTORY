...
시간축은 **가로(↔)와 세로(↕) 중에서 고를 수 있습니다.** 세로 보기에서는 y축이 연도가 되어
위에서 아래로 시간이 흐르고, 국가·주제 트랙이 세로 열로 나란히 서므로 휴대폰에서 특히 읽기 좋습니다.
...
| **시간축 방향** | 툴바의 `축: 가로 ↔ / 세로 ↕`. 세로를 고르면 **y축이 연도**가 되고 트랙이 세로 열로 선다 (보던 구간·필터는 그대로 유지) |

...
연표에는 **사건명만** 적혀 있습니다. 설명은 화면을 어지럽히지 않도록 빼 두었고,
항목을 **누르면 팝업**으로 전체 내용(기간·설명·연대 주석·출처)이 열립니다. 한 시기에 사건이 몰리면 **묶음(N건)** 으로 접히고,
...
THIRD-PARTY-NOTICES.md      제3자 구성요소 고지 (vis-timeline)
...
| `summary` | (선택) 세부 연표 팝업의 목록에 쓰이는 한 줄 요약, 60자 이내. 없으면 `description` 첫 문장을 줄여서 자동 생성 |
...
| **카테고리 선택** | 툴바의 `▤ 카테고리 n/11` 을 누르면 선택 패널이 열린다 — `전체 선택` / `전체 해제`, 트랙(왕조·국가사 / 주제사) 단위 체크(일부만 켜면 중간 표시), 카테고리별 체크와 항목 수. 패널 밖을 누르거나 `Esc` 로 닫는다 |
| 시대 이동 | `전체 / 고대 / 중세 / 근세 / 근현대` 버튼 한 번 |
| **카테고리 열** | 세로 보기에서 카테고리 하나가 같은 폭의 열 하나를 차지한다. 열에 다 담기지 않는 항목은 오른쪽 `+N` 배지로 접히고, 누르면 세부 연표가 열린다. 필터로 카테고리를 줄이면 열이 넓어진다 |
| **정보·라이선스** | 툴바 오른쪽 `ⓘ` — 내용 신뢰도, 데이터·소프트웨어 라이선스, 개인정보 고지 |
...
| **막대 표시** | 연표 막대에는 **사건명만** 적는다. 설명·요약은 팝업에서 본다 |
...
| vis-timeline 7.7.3 (`vendor/`) | Apache-2.0 **OR** MIT | 가능 | 저작권·라이선스 고지 유지 |

- **런타임 의존성이 없습니다.** `package.json` 에 dependencies 가 하나도 없어 전이 의존성 위험이 없습니다.
- **외부 호출이 없습니다.** CDN·웹폰트·분석 도구를 쓰지 않아 쿠키도, 제3자 데이터 전송도 없습니다.
  (`grep -rn "https\?://" index.html css js sw.js` 로 확인 가능)
- **웹폰트를 재배포하지 않습니다.** 기기에 이미 있는 시스템 글꼴만 지정합니다.
- **이미지가 없습니다.** `icons/` 는 이 저장소에서 직접 만든 것입니다.
- 앱 화면의 **`ⓘ 정보`** 팝업에 위 고지가 모두 들어 있어, 배포만으로 고지 의무가 충족됩니다.
...
2. EU·영국 이용자가 대상이면 **동의 배너(CMP)** 가 필요합니다. 한국만 대상이면 방침 고지로 충분한 경우가 많지만,
   광고 네트워크 약관을 확인하세요.
3. `ⓘ 정보` 팝업의 "외부로 나가는 요청이 없습니다" 문구를 **실제와 맞게 고쳐야 합니다.**

...
js/timeline.js              vis-timeline 래퍼 (연도↔Date 변환, 레인, 아이템)
js/app.js                   툴바·필터·검색·상세 패널 조립
...
| 확대·축소 | 마우스 휠 (또는 터치 핀치), 드래그로 좌우 이동 |
| 상세 보기 | 항목 클릭 → 오른쪽 패널(모바일은 하단 시트). 페이지 이동 없음 |
| 검토 대상만 보기 | `검토 전(draft)만` 체크 |
| 되돌리기 | `전체 초기화` 버튼 하나로 필터·검색·줌 모두 복구 (`Esc` 는 패널/검색 닫기) |
